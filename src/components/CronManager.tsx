...
import {
  Clock, Play, Pause, Lightning, CaretRight, ArrowsClockwise, Timer, Gear,
...
import { getCurrentAgent } from "../api";
...
export function CronManager() {
  ...
  const handleToggle = async (job: CronJob) => {
    ...
    try {
      ...
      showToast(`${!job.enabled ? "✅ Включено" : "⏸ Отключено"} ${job.name}`);
      await loadJobs();
    } catch (e: any) {
                ...
                >
                  ...
                  {selectedJob === job.id && (
                    <div className="px-4 pb-3 border-t" style={{ borderColor: "var(--border)" }}>
                      ...
                      {job.prompt ? (
                        <pre
                          className="text-xs px-3 py-2 rounded-lg overflow-y-auto whitespace-pre-wrap break-words"
                          style={{
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-[10px]" style={{ color: "var(--text-muted)" }}>
                          {job.deliver && (
                            <span className="px-1.5 py-0.5 rounded" style={{ background: "var(--bg-hover)" }}>
                              Доставка: {job.deliver}
                            </span>
                          )}
                          {job.sessionTarget && (
                            <span className="px-1.5 py-0.5 rounded" style={{ background: "var(--bg-hover)" }}>
                              Сессия: {job.sessionTarget}
                            </span>
                          )}
                        </div>
                      )}
          ...
          <div className="space-y-2">
                      ...
                      {sc.agents.map((agent) => (
                        ...
                        >
                          ...
                          <span style={{ color: agent.enabled ? "#22c55e" : "var(--text-muted)" }}>
                            {agent.heartbeat === "disabled" ? "Отключено" : agent.heartbeat}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            {systemCrons.length === 0 && (
              <div className="text-center py-12" style={{ color: "var(--text-muted)" }}>
                ...
                <p>Не удалось загрузить системные задачи</p>
              </div>
            )}
          </div>
        )}
